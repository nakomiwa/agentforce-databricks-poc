...
> **外部ログイン情報の「トークン更新の追加の状況コード」に `403` を入れること（必須）。**
> Databricks は期限切れトークンに 403 を返しますが、Salesforce は既定で 401 しか
> トークン更新の合図と見なしません。入れていないと、**1 時間ほど動いたあと
> 突然すべてが `HTTP 403: Invalid Token` になり、以後復旧しません。**
> 詳細は `claude/rootcause_403_invalid_token.md`。
